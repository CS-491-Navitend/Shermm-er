...
export class Game extends Scene {
  ...
  create(data) {
    ...
    let powerUpTimer = setInterval(() => {
      ...
      // use the sprite returned from spawnPowerUp
      powerUp = powerUpInfo;
      ...
      this.physics.add.overlap(
        ...
        powerUp,
        () => {
          const powerUp_kind = powerUp.getData("type"); // get the power-up type from sprite metadata
          if (powerUp_kind === 0) {
            // self service
            ...
            if (this.isBomb) {
              ...
              console.log("Shermie Data:", this.shermie.data.list);
            }
  ...
  update() {
    ...
    if (this.timer.timeRemaining % 3 === 0 && !this.shermieCooldown) {
  ...
  goalCollision() {
    ...
    this.timer.getBomb(this.shermie);
    this.goalColor.clear(true); // Clear goal rectangles
    this.gameLogic.goal();
    this.bonusFlag = false; // Reset bonus flag
  }
